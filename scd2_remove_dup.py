...
try:
    ...
    tb_columns = json_content[tb_nm]['column_names']
    tb_schema = json_content[tb_nm]['schema']
    ...
    logger.info("DataFrame with SCD Type 2 Columns:")
    ...
    if tb_nm not in db_table_list:
        ...
        updated_df = updated_df.withColumn("balance_uid", md5(concat(col("effectivedate"), lit("|"),
        col("subaccountid"), lit("|"),col("instrumentcode"), lit("|"),
        col("location"), lit("|"),col("custodianaccountid"), lit("|"),col("sourcesystemid"), lit("|"),
        col("positiontypeid"))))
        ...
        .select("loadflag", "effectivedate", "subaccountid", "instrumentcode", "location",
